...
bool CParagraphShadowNode::shouldNewRevisionDirtyMeasurement( // EDITED
    const ShadowNode& /*sourceShadowNode*/,
    const ShadowNodeFragment& fragment) const {
  return fragment.props != nullptr;
}

const Content& CParagraphShadowNode::getContent( // EDITED
...
Content CParagraphShadowNode::getContentWithMeasuredAttachments( // EDITED
...
void CParagraphShadowNode::setTextLayoutManager( // EDITED
...
Size CParagraphShadowNode::rawContentSize() { // EDITED
  return Size{
...
void CParagraphShadowNode::layout(LayoutContext layoutContext) { // EDITED
  ...
  auto paragraphShadowNode = static_cast<CParagraphShadowNode*>(this); // EDITED
  ...
  for (size_t i = 0; i < content.attachments.size(); i++) {
    ...
    paragraphShadowNode =
        static_cast<CParagraphShadowNode*>(paragraphOwningShadowNode.get()); // EDITED
  ...
  if (paragraphShadowNode != this) {
    this->children_ =
        static_cast<const CParagraphShadowNode*>(paragraphShadowNode)->children_; // EDITED
  }
}
